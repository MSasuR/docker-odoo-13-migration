...
RUN mkdir /var/tmp/openupgrade && mkdir /var/tmp/openupgrade/13.0
...
WORKDIR /var/tmp/openupgrade/13.0
COPY  /tmp/openupgrade/13.0 /var/tmp/openupgrade/13.0


RUN pip install --upgrade pip

RUN pip install openupgradelib
RUN pip install -r /openupgrade/13.0/requirements.txt
